...
### Fields
Typed, fluent builders: `Field::text()`, `::textarea()`, `::editor()`, `::select()`, `::image()`, `::file()`. Each field has a name, label, validation rules, default value, and type-specific props. They are serialised to JSON and sent to the Vue frontend to generate the correct input component.
...
```
config/          — ui-manager.php (routes, cache, discovery, media)
